...
using UnityEngine;
public class YuiTeam : AIBase
{

    public YuiTeam(int a_index) : base(a_index) { }
    ...
    protected override void Act(JObject state)
    {
        ...
        else if (tar_ene != null)
        {
            ...
            Move(x, z);
            if (int.Parse(me["skills"][1].ToString()) == 0 && Distance(me, tar_ene) < 5 * 5 && enemies.Count() > 1)
            {
                UseSkill(1);
                Move(x, z);
            }
            else if (Distance(me, tar_ene) <= shootRange)
            {
        ...
        else if (tar_barrel != null || tar_pick != null)
        {
            ...
            {
                if (Distance(me, tar_barrel) <= shootRange * shootRange && int.Parse(me["skills"][0].ToString()) == 0)
                {
                    var x = float.Parse(tar_barrel["pos"]["x"].ToString());
                    var z = float.Parse(tar_barrel["pos"]["z"].ToString());
                    UseSkill(0, x, z);
                    Move(x, z);
                }
                else
