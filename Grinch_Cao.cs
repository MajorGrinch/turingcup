...
using UnityEngine;
public class AnotherTeam : AIBase
{

    public AnotherTeam(int a_index) : base(a_index) { }
    ...
    protected override void Act(JObject state)
    {
        ...
        else if (tar_ene != null)
        {
            ...
            if (int.Parse(me["skills"][1].ToString()) == 0 && Distance(me, tar_ene) < shootRange * shootRange)
            {
        ...
        else if (tar_barrel != null || tar_pick != null)
        {
            ...
            {
                if(Distance(me,tar_barrel) <= 2.7*2.7*tyor_vel*tyor_vel*4 )
                    UseSkill(0);
